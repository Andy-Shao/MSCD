...
#     --adpt_wght_pth $BASE_PATH'/result/VocalSound/PANNs/TTDA' --forbid_ls 'END1' --freeze_pan --max_mode --wandb

python -m runs.VocalSound.PANNs.adaptation.teach_cons --dataset 'VocalSound' \
    --adpt_set_path $BASE_PATH'/data/Ada-VocalSound-C' \
    --eval_set_path $BASE_PATH'/data/VocalSound-C' \
    --corruption_level 'L1' --batch_size 32 --max_epoch 14 --num_of_shft 2 --fail_coll_lim 3 \
    --elect_weights '{"WHN":1.0, "ENQ":1.0, "END1":3.0, "END2":3.0, "ENSC":1.0, "PSH":1.0, "TST":1.0}' \
    --lrs '{"WHN":1e-4, "ENQ":1e-4, "END1":1e-4, "END2":1e-4, "ENSC":1e-4, "PSH":2e-4, "TST":1e-5}' \
    --adpt_wght_pth $BASE_PATH'/result/VocalSound/PANNs/TTDA' --freeze_pan --forbid_ls 'END1' --max_mode --wandb
