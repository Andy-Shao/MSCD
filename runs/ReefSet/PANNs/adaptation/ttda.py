...
from lib.component import Components, ReduceChannel, OneHot2Index, AudioPadding, TimeShift
...
if __name__ == '__main__':
    ...
    adpt_cp_rsc_set = MultiTFDataset(
        dataset=adpt_cp_rsc_set, 
        tfs=[
    ...
    adpt_loader = DataLoader(
        dataset=adpt_rsc_set, batch_size=args.batch_size, shuffle=False, drop_last=False,
        num_workers=args.num_workers
    )
    eval_loader = DataLoader(
        dataset=eval_rsc_set, batch_size=args.batch_size, shuffle=False, drop_last=False,
        num_workers=args.num_workers
    )
    ...
    for epoch in range(args.max_epoch+1):
        ...
        roc_auc = inference(args=args, pan=pan, clsf=clsf, data_loader=adpt_loader)
        print(f'ROC-AUC is: {roc_auc:.4f}, sample size is: {len(adpt_rsc_set)}')
        print('Evaluation Set')
        eval_roc_auc = inference(args=args, pan=pan, clsf=clsf, data_loader=eval_loader)
        print(f'ROC-AUC is: {eval_roc_auc:.4f}, sample size is: {len(eval_rsc_set)}')
        if max_roc <= roc_auc:
            max_roc = roc_auc
            store_weight(
        ...
        wandb_run.log(
            data={
                ...
                'Adaptation/ROC-AUC': roc_auc,
                'Adaptation/LR': learning_rate,
                'Adaptation/max ROC-AUC': max_roc,
                'Evaluation/ROC-AUC': eval_roc_auc,
