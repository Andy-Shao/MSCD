...
from runs.SpeechCommandsV2.PANNs.utils import build_model, teach_inference
...
def teacher_accu_analyzing(
    ...
    logger.log(data={f'Adaptation/{corruption_type} Accuracy': adpt_accus[corruption_type] for corruption_type in corruption_types}, step=step)
    ...
    eval_loader = DataLoader(
        dataset=eval_set, batch_size=args.batch_size, shuffle=False, drop_last=False, num_workers=args.num_workers
    )
    ...
    logger.log(data={f'Evaluation/{corruption_type} Accuracy': eval_accus[corruption_type] for corruption_type in corruption_types}, step=step)
...
if __name__ == '__main__':
    ...
    for epoch in range(args.max_epoch+1):
        ...
        for teach_pan in teach_pans: 
            teach_pan.train()
            # pan_freeze(pan=teach_pan, batch1d=True, batch2d=True) # do not freeze in No BN model
